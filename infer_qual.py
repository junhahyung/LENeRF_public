...
config_list = [
["/project/dataset/users/liam.1234/localnerf_logs/logs/eg3d_ffhq_clipedit_eyes/2022_09_20_14_28_28/config.yaml", "face_angry", 18000],
        ]
...
def main(args):
    ...
    for idx, (config_path, name, step) in enumerate(config_list):
        for w_i in [0,0.3,0.5,1,2]:
            configs = config.walk_configs(config_path)
            configs['ngpus'] = torch.cuda.device_count()

            configs['lambda']['ws_delta_lambda'] = w_i
            ...
            #if name == 'eyes_wink' or name == 'cat_odd_eyes':
            _name = name + f'w_i{w_i}'
            if 'cars' in _name:
                trainer.inference_qual(_name, zs, gen_shape=args.gen_shape, cars=True)
            elif  _name == 'cat_odd_eyes':
                trainer.inference_qual(_name, zs, half=True, gen_shape=args.gen_shape)
            else:
                trainer.inference_qual(_name, zs, gen_shape=args.gen_shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_shape', default=False)
